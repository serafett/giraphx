...
//            	vCount += partition.getVertices().size();
//            }
//            Log.info("SEREF Vertex number in the worker is: "+vCount);
            //**************************** SEREF-END ********************************//
         ...
         //**************************** SEREF-START ********************************//
                    if((token || !VERSION_OF_JOB.equals("tGiraphx_coloring")) 
                    		&& basicVertex.needsOperation){
         ...
         //**************************** SEREF-START ********************************//
                        if((vertexCounter++)%5000 == 0){
	                        ...
	                        incrementUsedMemory(context);
                        }
                        basicVertex.compute(vertexMsgIt);                     
                        basicVertex.releaseResources();
                    }
                    else{
                    	incrementHaltedPerSuperstep(context);
                    }
        ...
        //**************************** SEREF-START ********************************//
                for (BasicVertex<I, V, E, M> basicVertex :
                    partition.getVertices()) {
//                	Log.info("Halt-NeedsOperation: "+basicVertex.isHalted()
//                    		+basicVertex.needsOperation);
    ...
    //**************************** SEREF-START ********************************//
    public void incrementHaltedPerSuperstep(Context context){      
    	long ss = serviceWorker.getSuperstep();
    	String ctr_name = "Halted vertex count at Superstep "+ss;
	    String GIRAPH_TIMERS_COUNTER_GROUP_NAME = "Giraph Stats";
			context.getCounter(
	            GIRAPH_TIMERS_COUNTER_GROUP_NAME,
	            ctr_name).increment(1);
    }
    public void incrementUsedMemory(Context context){      
    	long ss = serviceWorker.getSuperstep();
    	String ctr_name = "Used Memory at Superstep "+ss;
    	long usedMem = (long) (Runtime.getRuntime().totalMemory() / 1024f / 1024f 
    			- Runtime.getRuntime().freeMemory() / 1024f / 1024f);
	    String GIRAPH_TIMERS_COUNTER_GROUP_NAME = "Giraph Stats";
		long oldVal = context.getCounter(
	            GIRAPH_TIMERS_COUNTER_GROUP_NAME,
	            ctr_name).getValue();
		if(usedMem > oldVal){
			context.getCounter(
		            GIRAPH_TIMERS_COUNTER_GROUP_NAME,
		            ctr_name).increment(usedMem-oldVal);
		}
    }
    //**************************** SEREF-END ********************************//
}
